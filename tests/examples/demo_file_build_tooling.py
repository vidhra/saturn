...
async def demo_file_operations():
    ...
    config = {"working_directory": "demo_workspace"}
    ...
    for op in file_operations:
        success, result = await executor.execute(
            ...
            op["params"],
            console,
            f"file_op_{op['params']['file_path']}",
        )
...
async def demo_project_builds():
    ...
    success, result = await executor.execute(
        "detect_project", {}, console, "detect_demo"
    )
...
setup(
    ...
    success, result = await executor.execute(
        "build_project", {"project_type": "python"}, console, "python_build"
    )
    ...
    success, result = await executor.execute(
        "detect_project", {}, console, "detect_node"
    )
...
int main() {
    ...
    success, result = await executor.execute(
        "detect_project", {}, console, "final_detect"
    )
...
"""
    ...
    for op in copy_operations:
        success, result = await executor.execute(
            "copy_file", op, console, f"batch_copy_{op['source'].replace('.', '_')}"
        )
...
def display_summary():
    ...
    table.add_row("File Operations", "read, write, list, copy, template", "✅ Complete")

    table.add_row(
        "Docker Operations", "build, run, compose, generate dockerfile", "✅ Complete"
    )
