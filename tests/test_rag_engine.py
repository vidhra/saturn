...
try:
    ...
    for env_path in env_file_paths:
        if env_path.exists():
            ...
            print(
                f"📄 [RAG Test] Loaded environment variables from: {env_path.absolute()}"
            )
            env_loaded = True
            break
    ...
    if not env_loaded:
        print(
            "⚠️  [RAG Test] No .env file found. Using system environment variables only."
        )

except ImportError:
    print(
        "⚠️  [RAG Test] python-dotenv not installed. Using system environment variables only."
    )
...
from saturn.rag_engine import (DEFAULT_EMBED_MODEL_NAME, CLIContextAwareParser,
                               RAGEngine, build_provider_db_config)
...
"""
            ...
            for i, node in enumerate(nodes):
                ...
                console.print(
                    f"  Node {i+1}: '{metadata.get('section_title', 'Unknown')}' "
    ...
    async def test_in_memory_rag(
        ...
        try:
            ...
            # Test query
            result = await rag_engine.query_docs("How do I create a compute instance?")
    ...
    async def test_gemini_embeddings(self) -> bool:
        ...
        if not self.google_api_key:
            console.print(
                "⚠️  Skipping Gemini embeddings test - GOOGLE_API_KEY not found"
            )
        ...
        try:
            ...
            # Test query
            result = await rag_engine.query_docs("How do I list storage buckets?")
            ...
            if result and ("storage" in result.lower() or "bucket" in result.lower()):
                ...
                console.print(
                    Panel(
                        result[:300] + "...",
                        title="Gemini Query Result Preview",
                ...
                console.print(
                    "❌ Gemini embeddings RAG query returned unexpected result"
                )
                self.test_results["gemini_embeddings"] = False
                return False
    ...
    async def test_chroma_vector_store(self) -> bool:
        ...
        try:
            ...
            success = rag_engine.ingest_and_build_index(
                documents_path=self.test_docs_dir, force_rebuild=True
            )
            ...
            # Test query
            result = await rag_engine.query_docs(
                "How to create compute instances with machine type?"
            )
            ...
            if result and "machine-type" in result:
                ...
                console.print(
                    Panel(
                        result[:300] + "...",
                        title="ChromaDB Query Result Preview",
                        border_style="blue",
                    )
                )
                ...
                console.print(
                    "❌ ChromaDB vector store query returned unexpected result"
                )
                self.test_results["chroma_vector_store"] = False
                return False
    ...
    async def test_duckdb_vector_store(self) -> bool:
        ...
        try:
            ...
            # Test query
            result = await rag_engine.query_docs("List all instances in a zone")
            ...
            if result and ("list" in result.lower() or "zone" in result.lower()):
                ...
                console.print(
                    Panel(
                        result[:300] + "...",
                        title="DuckDB Query Result Preview",
                        border_style="cyan",
                    )
                )
    ...
    async def test_provider_specific_configs(self) -> bool:
        ...
        try:
            ...
            gcp_chroma_config = build_provider_db_config(
                self.test_config, "gcp", "chroma"
            )
            aws_chroma_config = build_provider_db_config(
                self.test_config, "aws", "chroma"
            )

            gcp_duckdb_config = build_provider_db_config(
                self.test_config, "gcp", "duckdb"
            )
            aws_duckdb_config = build_provider_db_config(
                self.test_config, "aws", "duckdb"
            )

            # Verify configurations are different
            assert (
                gcp_chroma_config["chroma_collection_name"]
                != aws_chroma_config["chroma_collection_name"]
            )
            assert (
                gcp_duckdb_config["duckdb_table_name"]
                != aws_duckdb_config["duckdb_table_name"]
            )

            console.print("✅ Provider-specific configurations test passed")
            console.print(
                f"  GCP Chroma: {gcp_chroma_config['chroma_collection_name']}"
            )
            console.print(
                f"  AWS Chroma: {aws_chroma_config['chroma_collection_name']}"
            )
    ...
    async def test_query_similarity_filtering(self) -> bool:
        ...
        try:
            ...
            result_high = await rag_engine.query_docs(
                "create compute instance", min_similarity_score=0.1
            )

            # Test with very high similarity threshold (should return fewer/no results)
            result_low = await rag_engine.query_docs(
                "create compute instance", min_similarity_score=0.95
            )
            ...
            if has_results_high:
                ...
                console.print(
                    "❌ Query similarity filtering test failed - no results with low threshold"
                )
                self.test_results["similarity_filtering"] = False
                return False
