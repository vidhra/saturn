...
from .graphrag_system import (COMMON_ASSET_TYPES, get_gcp_assets,
                              infer_relationships, transform_asset_to_node)


class GraphRAGAssetManager:
    """Manages asset discovery and monitoring using GraphRAG approach with local NetworkX graph storage."""
    ...
    def __init__(
        ...
        """
        Initialize the GraphRAG Asset Manager.
    ...
    def discover_assets(
        ...
        assets = get_gcp_assets(
            scope, asset_types or COMMON_ASSET_TYPES, self.gcp_creds_path
        )
        ...
        # Build relationships between nodes using existing code
    ...
    def _save_graph_data(
        ...
        self.logger.info(
            f"Saved {len(nodes)} nodes and {len(relationships)} relationships"
        )

    def _update_rag_engine(
        self, nodes: List[Dict[str, Any]], relationships: List[Dict[str, Any]]
    ):
        ...
        for node in nodes:
            ...
            incoming = list(self.graph.predecessors(node["id"]))

            # Build context from graph structure
            context = []
            for neighbor in neighbors:
                edge_data = self.graph.get_edge_data(node["id"], neighbor)
                context.append(
                    f"Connects to {neighbor} via {edge_data['relation_type']}"
                )
            for pred in incoming:
                edge_data = self.graph.get_edge_data(pred, node["id"])
                context.append(
                    f"Connected from {pred} via {edge_data['relation_type']}"
                )
        ...
        for rel in relationships:
            source_node = self.graph.nodes[rel["source_id"]]
            target_node = self.graph.nodes[rel["target_id"]]
    ...
    async def monitor_asset(self, asset_id: str) -> Dict[str, Any]:
        """
        ...
        if asset_id in self.graph:
            ...
            neighbors = list(self.graph.neighbors(asset_id))
            incoming = list(self.graph.predecessors(asset_id))

            # Build graph context
            context = []
            for neighbor in neighbors:
                edge_data = self.graph.get_edge_data(asset_id, neighbor)
                neighbor_data = self.graph.nodes[neighbor]
                context.append(
                    f"Connects to {neighbor_data.get('display_name', neighbor)} via {edge_data['relation_type']}"
                )
            for pred in incoming:
                edge_data = self.graph.get_edge_data(pred, asset_id)
                pred_data = self.graph.nodes[pred]
                context.append(
                    f"Connected from {pred_data.get('display_name', pred)} via {edge_data['relation_type']}"
                )
        ...
        response = await self.rag_engine.query_docs(query)
    ...
    def get_asset_relationships(self, asset_id: str) -> List[Dict[str, Any]]:
        ...
        for neighbor in self.graph.neighbors(asset_id):
            edge_data = self.graph.get_edge_data(asset_id, neighbor)
                ...
                result.append(
                    {
                        "from": {
                            "id": current,
                            "type": self.graph.nodes[current].get("asset_type"),
                            "name": self.graph.nodes[current].get("display_name"),
                        },
                        "to": {
                            "id": next_node,
                            "type": self.graph.nodes[next_node].get("asset_type"),
                            "name": self.graph.nodes[next_node].get("display_name"),
                        },
