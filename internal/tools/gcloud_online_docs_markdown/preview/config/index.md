# gcloud preview config  |  Google Cloud CLI Documentation

*Source: [https://cloud.google.com/sdk/gcloud/reference/preview/config](https://cloud.google.com/sdk/gcloud/reference/preview/config)*
...
: **`gcloud preview config` `[GROUP](https://cloud.google.com/sdk/gcloud/reference/preview/config#GROUP)` | `[COMMAND](https://cloud.google.com/sdk/gcloud/reference/preview/config#COMMAND)` [`[GCLOUD_WIDE_FLAG](https://cloud.google.com/sdk/gcloud/reference/preview/config#GCLOUD-WIDE-FLAGS) …`]**

**DESCRIPTION**

: `(PREVIEW)` The gcloud preview config command group lets you set,
view and unset properties used by Google Cloud CLI.
A configuration is a set of properties that govern the behavior of `[gcloud](https://cloud.google.com/sdk/gcloud/reference)` and other Google Cloud CLI tools.
The initial `default` configuration is set when `[gcloud init](https://cloud.google.com/sdk/gcloud/reference/init)` is run. You can create
additional named configurations using `[gcloud init](https://cloud.google.com/sdk/gcloud/reference/init)` or `[gcloud preview
config configurations create](https://cloud.google.com/sdk/gcloud/reference/preview/config/configurations/create)`.
To display the path of the active configuration along with information about the
current `[gcloud](https://cloud.google.com/sdk/gcloud/reference)` environment, run
$ [gcloud info](https://cloud.google.com/sdk/gcloud/reference/info).
To switch between configurations, use `[gcloud
preview config configurations activate](https://cloud.google.com/sdk/gcloud/reference/preview/config/configurations/activate)`.
gcloud supports several flags that have the same effect as properties in a
configuration (for example, gcloud supports both the `--project` flag
and `project` property). Properties differ from flags in that flags
affect command behavior on a per-invocation basis. Properties allow you to
...
``project`` property in the
``core`` category to
``my-project``, use a command like:

```
CLOUDSDK_CORE_PROJECT=my-project gcloud config get core/project
```

For more information on configurations, see `[gcloud topic
...
**`[configurations](https://cloud.google.com/sdk/gcloud/reference/preview/config/configurations)`**:
...
**`[get](https://cloud.google.com/sdk/gcloud/reference/preview/config/get)`**:
`(PREVIEW)` Print the value of a Google Cloud CLI property.

**`[list](https://cloud.google.com/sdk/gcloud/reference/preview/config/list)`**:
`(PREVIEW)` List Google Cloud CLI properties for the currently active
configuration.

**`[set](https://cloud.google.com/sdk/gcloud/reference/preview/config/set)`**:
`(PREVIEW)` Set a Google Cloud CLI property.

**`[unset](https://cloud.google.com/sdk/gcloud/reference/preview/config/unset)`**:
