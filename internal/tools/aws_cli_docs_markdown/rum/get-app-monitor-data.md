# get-app-monitor-dataÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/rum/get-app-monitor-data.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/rum/get-app-monitor-data.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/rum-2018-05-10/GetAppMonitorData)

`get-app-monitor-data` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
...
Specifying `Invert` as the `Name` works as a ânot equal toâ filter. For example, specify `Invert` as the `Name` and specify `Chrome` as the value to return all events except events from user sessions with the Chrome browser.
