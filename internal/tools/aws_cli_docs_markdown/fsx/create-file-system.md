...
*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/fsx/create-file-system.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/fsx/create-file-system.html)*
...
If a file system with the specified client request token exists and the parameters match, `CreateFileSystem` returns the description of the existing file system. If a file system with the specified client request token exists and the parameters donât match, this call returns `IncompatibleParameterError` . If a file system with the specified client request token doesnât exist, `CreateFileSystem` does the following:
...
The `CreateFileSystem` call returns while the file systemâs lifecycle state is still `CREATING` . You can check the file-system creation status by calling the [DescribeFileSystems](https://docs.aws.amazon.com/fsx/latest/APIReference/API_DescribeFileSystems.html) operation, which returns the file system state along with other information.

See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/fsx-2018-03-01/CreateFileSystem)
...
Sets the storage capacity of the file system that youâre creating, in gibibytes (GiB).
...
Sets the storage class for the file system that youâre creating. Valid values are `SSD` , `HDD` , and `INTELLIGENT_TIERING` .
...
For Windows `SINGLE_AZ_1` and `SINGLE_AZ_2` and all Lustre deployment types, provide exactly one subnet ID. The file server is launched in that subnetâs Availability Zone.
...
A list of IDs specifying the security groups to apply to all network interfaces created for file system access. This list isnât returned in later requests to describe the file system.
...
The tags to apply to the file system thatâs being created. The key value of the `Name` tag appears in the console as the file system name.
...
The Microsoft Windows configuration for the file system thatâs being created.
...
Sets the throughput capacity for the file system that youâre creating in megabytes per second (MBps). For more information, see [Managing throughput capacity](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/managing-throughput-capacity.html) in the FSx for ONTAP User Guide.
...
For FSx for Lustre file systems, sets the Lustre version for the file system that youâre creating. Valid values are `2.10` , `2.12` , and `2.15` :
