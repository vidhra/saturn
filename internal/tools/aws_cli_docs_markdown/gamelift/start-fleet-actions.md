# start-fleet-actionsÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/gamelift/start-fleet-actions.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/gamelift/start-fleet-actions.html)*
...
Resumes certain types of activity on fleet instances that were suspended with [StopFleetActions](https://docs.aws.amazon.com/gamelift/latest/apireference/API_StopFleetActions.html) . For multi-location fleets, fleet actions are managed separately for each location. Currently, this operation is used to restart a fleetâs auto-scaling activity.

This operation can be used in the following ways:

- To restart actions on instances in the fleetâs home Region, provide a fleet ID and the type of actions to resume.
- To restart actions on instances in one of the fleetâs remote locations, provide a fleet ID, a location name, and the type of actions to resume.

If successful, Amazon GameLift once again initiates scaling events as triggered by the fleetâs scaling policies. If actions on the fleet location were never stopped, this operation will have no effect.
