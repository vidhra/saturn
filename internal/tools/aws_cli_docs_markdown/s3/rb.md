...
*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/rb.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/rb.html)*
...
The following `rb` command removes a bucket.  In this example, the userâs bucket is `amzn-s3-demo-bucket`.  Note that the bucket must be empty in order to remove:
...
remove the bucket itself.  In this example, the userâs bucket is `amzn-s3-demo-bucket` and the objects in `amzn-s3-demo-bucket` are
