# failover-global-clusterÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/rds/failover-global-cluster.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/rds/failover-global-cluster.html)*
...
Promotes the specified secondary DB cluster to be the primary DB cluster in the global database cluster to fail over or switch over a global database. Switchover operations were previously called âmanaged planned failovers.â
...
- Failing over - Specify the `AllowDataLoss` parameter and donât specify the `Switchover` parameter.
- Switching over - Specify the `Switchover` parameter or omit it, but donât specify the `AllowDataLoss` parameter.
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/rds-2014-10-31/FailoverGlobalCluster)
...
The identifier of the global database cluster (Aurora global database) this operation should apply to. The identifier is the unique key assigned by the user when the Aurora global database is created. In other words, itâs the name of the Aurora global database.
