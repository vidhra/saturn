...
*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iotsitewise/list-time-series.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iotsitewise/list-time-series.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/iotsitewise-2019-12-02/ListTimeSeries)

`list-time-series` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
When using `--output text` and the `--query` argument on a paginated response, the `--query` argument must extract data from the results of the following query expressions: `TimeSeriesSummaries`
...
- `ASSOCIATED` â The time series is associated with an asset property.
- `DISASSOCIATED` â The time series isnât associated with any asset property.
