...
*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/connectcases/delete-field.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/connectcases/delete-field.html)*
...
- Calling `GetCase` with a deleted field ID returns the deleted fieldâs value if one exists.
- Calling `UpdateCase` with a deleted field ID throws a `ValidationException` if the case does not already contain a value for the deleted field. Otherwise it succeeds, allowing you to update or remove (using `emptyValue: {}` ) the fieldâs value from the case.
...
- Calling `SearchCases` with a `searchTerm` value that matches a deleted fieldâs value on a case returns the case in the response.
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/connectcases-2022-10-03/DeleteField)
