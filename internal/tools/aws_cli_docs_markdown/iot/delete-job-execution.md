# delete-job-executionÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iot/delete-job-execution.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iot/delete-job-execution.html)*
...
(Optional) When true, you can delete a job execution which is âIN_PROGRESSâ. Otherwise, you can only delete a job execution which is in a terminal state (âSUCCEEDEDâ, âFAILEDâ, âREJECTEDâ, âREMOVEDâ or âCANCELEDâ) or an exception will occur. The default is false.

### Note

Deleting a job execution which is âIN_PROGRESSâ, will cause the device to be unable to access job information or update the job execution status. Use caution and ensure that the device is able to recover to a valid state.
