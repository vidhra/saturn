# forecast-geofence-eventsÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/location/forecast-geofence-events.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/location/forecast-geofence-events.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/location-2020-11-19/ForecastGeofenceEvents)

`forecast-geofence-events` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
When using `--output text` and the `--query` argument on a paginated response, the `--query` argument must extract data from the results of the following query expressions: `ForecastedEvents`
...
The deviceâs state, including current position and speed.

Position -> (list)

The deviceâs position.
...
The closest distance from the deviceâs position to the geofence.
