# start-flow-executionÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/bedrock-agent-runtime/start-flow-execution.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/bedrock-agent-runtime/start-flow-execution.html)*
...
This operation returns an Amazon Resource Name (ARN) that you can use to track and manage your flowâs async execution.

### Note

Asynchronous flows is in preview release for Amazon Bedrock and is subject to change.

See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/bedrock-agent-runtime-2023-07-26/StartFlowExecution)

`start-flow-execution` uses document type values. Document types follow the JSON data model where valid values are: strings, numbers, booleans, null, arrays, and objects. For command input, options and nested parameters that are labeled with the type `document` must be provided as JSON. Shorthand syntax does not support document types.
...
`--flow-execution-name` (string)

The unique name for the async execution. If you donât provide one, a system-generated name is used.
...
```
content={},nodeInputName=string,nodeName=string,nodeOutputName=string ...
```
...
`--model-performance-configuration` (structure)
