# authorize-snapshot-accessÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/redshift/authorize-snapshot-access.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/redshift/authorize-snapshot-access.html)*
...
- *If the snapshot to access doesnât exist and the associated IAM policy doesnât allow access to all (*) snapshots* - This parameter is required. Otherwise, permissions arenât available to check if the snapshot exists.
- *If the snapshot to access exists* - This parameter isnât required. Redshift can retrieve the cluster identifier and use it to validate snapshot authorization.
