# nat-gateway-deletedÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ec2/wait/nat-gateway-deleted.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ec2/wait/nat-gateway-deleted.html)*
...
Wait until JMESPath query NatGateways[].State returns deleted for all elements when polling with `describe-nat-gateways`. It will poll every 15 seconds until a successful state has been reached. This will exit with a return code of 255 after 40 failed checks.

See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/ec2-2016-11-15/DescribeNatGateways)

`nat-gateway-deleted` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
