...
*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ivs/list-streams.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ivs/list-streams.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/ivs-2020-07-14/ListStreams)
...
When using `--output text` and the `--query` argument on a paginated response, the `--query` argument must extract data from the results of the following query expressions: `streams`
...
The streamâs state. Do not rely on the `OFFLINE` state, as the API may not return it; instead, a âNotBroadcastingâ error will indicate that the stream is not live.
