# get-clusters-for-imageÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/inspector2/get-clusters-for-image.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/inspector2/get-clusters-for-image.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/inspector2-2020-06-08/GetClustersForImage)

`get-clusters-for-image` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
When using `--output text` and the `--query` argument on a paginated response, the `--query` argument must extract data from the results of the following query expressions: `cluster`
...
clusterMetadata -> (tagged union structure)

The metadata for a cluster.

### Note

This is a Tagged Union structure. Only one of the following top level keys can be set: `awsEcsMetadataDetails`, `awsEksMetadataDetails`.

awsEcsMetadataDetails -> (structure)
...
awsEksMetadataDetails -> (structure)
