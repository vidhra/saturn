# update-user-profileÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/opsworks/update-user-profile.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/opsworks/update-user-profile.html)*
...
The user IAM ARN. This can also be a federated userâs ARN.

`--ssh-username` (string)

The userâs SSH user name. The allowable characters are [a-z], [A-Z], [0-9], â-â, and â_â. If the specified name includes other punctuation marks, OpsWorks Stacks removes them. For example, `my.name` will be changed to `myname` . If you do not specify an SSH user name, OpsWorks Stacks generates one from the IAM user name.

`--ssh-public-key` (string)

The userâs new SSH public key.
