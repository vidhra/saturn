# list-retained-messagesÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iot-data/list-retained-messages.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/iot-data/list-retained-messages.html)*
...
This action returns only the topic names of the retained messages. It doesnât return any message payloads. Although this action doesnât return a message payload, it can still incur messaging costs.
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/iot-data-2015-05-28/ListRetainedMessages)
...
`list-retained-messages` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
When using `--output text` and the `--query` argument on a paginated response, the `--query` argument must extract data from the results of the following query expressions: `retainedTopics`
...
A summary list the accountâs retained messages. The information returned doesnât include the message payloads of the retained messages.
...
The size of the retained messageâs payload in bytes.
