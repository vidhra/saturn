# cancel-key-deletionÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/kms/cancel-key-deletion.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/kms/cancel-key-deletion.html)*
...
When the `cancel-key-deletion` command succeeds, the scheduled deletion is canceled. However, the key state of the KMS key is `Disabled`, so you canât use the KMS key in cryptographic operations. To restore its functionality, use the `enable-key` command .
