...
An object that contains the properties for the Amazon ECS resources of a job.When `ecsProperties` is used in the job definition, it canât be used in addition to `containerProperties` , `eksProperties` , or `nodeProperties` .
...
An object with properties that are specific to Amazon EKS-based jobs. When `eksProperties` is used in the job definition, it canât be used in addition to `containerProperties` , `ecsProperties` , or `nodeProperties` .
