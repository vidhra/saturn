# list-operation-eventsÂ¶

*Source: [https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ssm-sap/list-operation-events.html](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ssm-sap/list-operation-events.html)*
...
See also: [AWS API Documentation](https://docs.aws.amazon.com/goto/WebAPI/ssm-sap-2018-05-10/ListOperationEvents)

`list-operation-events` is a paginated operation. Multiple API calls may be issued in order to retrieve the entire data set of results. You can disable pagination by providing the `--no-paginate` argument.
...
```
Name=string,Value=string,Operator=string ...
```
...
A description of the operation event. For example, âStop the EC2 instance i-abcdefgh987654321â.
...
Example of `ResourceArn` : â`arn:aws:ec2:us-east-1:111111111111:instance/i-abcdefgh987654321` â

ResourceType -> (string)

The resource type.

Example of `ResourceType` : â`AWS::SystemsManagerSAP::Component` â or â`AWS::EC2::Instance` â.
