  ...
  "AudioDescriptions": [
    {
      ...
      "StreamName": "string",
      "AudioDashRoles": ["ALTERNATE"|"COMMENTARY"|"DESCRIPTION"|"DUB"|"EMERGENCY"|"ENHANCED-AUDIO-INTELLIGIBILITY"|"KARAOKE"|"MAIN"|"SUPPLEMENTARY", ...],
  ...
  "CaptionDescriptions": [
    {
      ...
      "LanguageDescription": "string",
      "Name": "string",
      "CaptionDashRoles": ["ALTERNATE"|"CAPTION"|"COMMENTARY"|"DESCRIPTION"|"DUB"|"EASYREADER"|"EMERGENCY"|"FORCED-SUBTITLE"|"KARAOKE"|"MAIN"|"METADATA"|"SUBTITLE"|"SUPPLEMENTARY", ...],
      "DvbDashAccessibility": "DVBDASH_1_VISUALLY_IMPAIRED"|"DVBDASH_2_HARD_OF_HEARING"|"DVBDASH_3_SUPPLEMENTAL_COMMENTARY"|"DVBDASH_4_DIRECTORS_COMMENTARY"|"DVBDASH_5_EDUCATIONAL_NOTES"|"DVBDASH_6_MAIN_PROGRAM"|"DVBDASH_7_CLEAN_FEED"
    }
    ...
  ],
...
List of input attachments for channel.(structure)
...
Specification of network and file inputs for this channelCodec -> (string)
...
Maintenance settings for this channel.MaintenanceDay -> (string)
...
`--reserved` (string)
Deprecated field thatâs only usable by whitelisted customers.
...
`--vpc` (structure)
Settings for the VPC outputsPublicAddressAllocationIds -> (list)
...
`--anywhere-settings` (structure)
The Elemental Anywhere settings for this channel.ChannelPlacementGroupId -> (string)
...
```
ChannelPlacementGroupId=string,ClusterId=string
```
...
`--channel-engine-version` (structure)
The desired engine version for this channel.Version -> (string)

The build identifier of the engine version to use for this channel. Specify âDEFAULTâ to reset to the default version.
...
All the parameters that apply to this example channel but that arenât specified in this file will either take the default value, be set to null, or take a unique value generated by MediaLive.
...
- `OutputGroups`, `Outputs`. These show all the parameters for the output group and outputs, including those that you didnât include but that are relevant to this channel. The parameters might be empty (perhaps indicating the parameter or feature is disabled in this channel configuration) or might show the default value that will apply.
