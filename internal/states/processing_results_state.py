    ...
    async def run(
        ...
        if not execution_results:
            ...
            if not context.current_errors:
                ...
                print(
                    "No tools executed, but prior errors existed. Assuming failure to correct."
                )
        ...
        for result_tuple in execution_results:
            ...
            if isinstance(result_tuple, Exception):
                print(
                    f"  [Error] Encountered exception from execution task: {result_tuple}"
                )

                tool_name = "unknown_tool (task exception)"
                error_details = str(result_tuple)
                current_errors.append(
                    {"method": tool_name, "error": error_details, "arguments": {}}
                )
                has_errors = True
            ...
            if success:
                ...
                current_errors.append(
                    {"method": tool_name, "error": str(result), "arguments": args_used}
                )
        ...
        if has_errors:
            print(
                f"Encountered {len(current_errors)} error(s) during execution. Transitioning to ERROR_HANDLING."
            )
