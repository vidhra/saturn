...
from internal.dag.dag import AcyclicGraph, Edge, ORDER_UP
...
class PlanningState(BaseState):
    ...
    async def run(self, context: StateMachineContext) -> Tuple[Type[BaseState], StateMachineContext]:
        ...
        while attempt <= max_attempts:
            ...
            file_tool_caller = FileBuildToolCaller(context.file_build_executor.config.get('working_directory', '.'))
            file_tool_schemas = file_tool_caller.get_tools_schema()
            context.file_tool_names = set([tool['function']['name'] for tool in file_tool_schemas])
            
            print(f"Available file tool names: {context.file_tool_names}")
            print(f"execute_command in file_tool_names: {'execute_command' in context.file_tool_names}")
            ...
            context.state_recorder.record_event("plan_generation_success", {
    ...
    async def _generate_plan_dag(
        ...
        """
        if console:
            console.print(f"Generating execution plan for query: [cyan]{user_query}[/cyan]")
        ...
        available_cloud_tools = "cli_command_generation (for gcp and aws cloud operations)"
        ...
        try:
            ...
            for step in plan_steps:
                ...
                if console:
                    console.print(f"[dim]DEBUG: Validating step {step.get('id')} with tool '{tool_to_use}', cloud_provider: {cloud_provider}[/dim]")
                    console.print(f"[dim]DEBUG: file_tool_names = {file_tool_names}[/dim]")
                if not cloud_provider and tool_to_use in file_tool_names:
                    ...
                    if console:
                        console.print(f"[bold red]Error:[/] {error_msg}")
                    state_logger.record_event("plan_generation_failed", {"error": error_msg, "plan_steps": plan_steps, "available_tools": list(file_tool_names)})
                    return None, None
