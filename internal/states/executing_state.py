...
class ExecutingState(BaseState):
    ...
    async def run(self, context: StateMachineContext) -> Tuple[Type[BaseState], StateMachineContext]:
        ...
        if console:
            console.print(f"Executing DAG with {len(context.execution_order)} steps in order: {' -> '.join(context.execution_order)}")
        ...
        for step_id in context.execution_order:
            ...
            step_dependencies = [edge.source for edge in context.dag.edges if edge.target == step_id]
            contextual_outputs = {}
            for dep_id in step_dependencies:
                if dep_id in context.step_outputs:
                    ...
                    if dep_details.get("pass_output_to_next", True):
                        if isinstance(context.step_outputs[dep_id], dict) and 'error' in context.step_outputs[dep_id]:
                            ...
                            contextual_outputs[dep_id] = {"status": "SUCCESS", "output": context.step_outputs[dep_id]}
                elif dep_id in context.step_details_map and not context.step_details_map[dep_id].get("pass_output_to_next", True):
                    if console:
            ...
            if tool_to_use in context.file_tool_names or (cloud_provider is None or str(cloud_provider).lower() == "none"):
            ...
            if not step_success:
                ...
                if isinstance(step_result, dict) and "error" in step_result:
                    accumulated_errors.append({"step_id": step_id, "error": step_result["error"]})
                else:
                    accumulated_errors.append({"step_id": step_id, "error": str(step_result)})
                break
        ...
        if all_steps_succeeded and not accumulated_errors:
            ...
            if console:
                console.print(Panel(f"[bold red]Execution failed. See errors above or in logs.[/bold red]", border_style="red"))
            context.current_errors.extend(accumulated_errors)
            context.state_recorder.set_final_run_status("FAILED_AT_STEP", accumulated_errors)
            return FailedState, context

    async def _execute_file_tool_step(self, step_id: str, step_details: Dict[str, Any], context: StateMachineContext, console) -> Tuple[bool, Any]:
        ...
        try:
            ...
            context.state_recorder.record_node_result(
                step_id, success, actual_result, 
                "COMPLETED_FILE_TOOL" if success else "FAILED_FILE_TOOL"
            )
            
            if not success:
                error_msg = actual_result.get("error", "Unknown error") if isinstance(actual_result, dict) else str(actual_result)
                if console:
                    console.print(f"[bold red]File tool step {step_id} failed: {error_msg}[/bold red]")
            ...
            context.state_recorder.record_node_result(step_id, False, {"error": error_msg}, "FAILED_FILE_TOOL_EXCEPTION")
            return False, {"error": error_msg, "step_id": step_id}
    ...
    async def _execute_dag_step(
        ...
        context.state_recorder.record_event("step_execution_start", {"step_id": step_id, "description": step_details.get('description')})
        ...
        if cloud_provider == "gcp":
            ...
            if not context.gcp_executor:
                ...
                context.state_recorder.record_node_result(step_id, False, {"error": error_msg}, "FAILED_EXECUTOR_MISSING")
                return False, {"error": error_msg, "step_id": step_id}
            ...
            if console:
                console.print(f"[bold red]Error:[/] {error_msg}")
            context.state_recorder.record_node_result(step_id, False, {"error": error_msg}, "FAILED_UNKNOWN_PROVIDER")
            return False, {"error": error_msg, "step_id": step_id}
        ...
        if context.rag_engine and context.rag_engine.query_engine:
            ...
            if console:
                console.print("[yellow]Warning: RAG engine instance not provided to step execution.[/yellow]")
        else:
            if console:
        ...
        while attempt < max_attempts:
            ...
            context.state_recorder.record_event("step_attempt", {"step_id": step_id, "attempt_number": attempt})
            ...
            try:
                ...
                if not command_to_execute:
                    ...
                    context.state_recorder.record_event("step_attempt_failed_empty_command", {"step_id": step_id, "attempt": attempt})
                    if attempt >= max_attempts:
                        parsed_command_for_log = _parse_command(command_to_execute) if command_to_execute else {"original_command": "EMPTY"}
                        context.state_recorder.record_node_result(step_id, False, {"error": last_error, "parsed_command": parsed_command_for_log }, "FAILED_EMPTY_COMMAND")
                ...
                # Execute the command
                if cloud_provider == "gcp":
                    success, result_or_error = await context.gcp_executor.execute(command_to_execute, console, step_id)
                elif cloud_provider == "aws":
                    success, result_or_error = await context.aws_executor.execute(command_to_execute, console, step_id)
                ...
                if success:
                    ...
                    context.state_recorder.record_node_result(step_id, True, success_payload, "COMPLETED_SUCCESS")
                ...
                context.state_recorder.record_event("step_execution_exception_attempt", {"step_id": step_id, "attempt": attempt, "error": last_error, "traceback": traceback.format_exc()})
        ...
        context.state_recorder.record_node_result(step_id, False, {"error": last_error, "final_attempt_command_str": command_to_execute, "parsed_command": parsed_command_for_log}, "FAILED_MAX_ATTEMPTS")
        return False, {"error": last_error, "step_id": step_id} 