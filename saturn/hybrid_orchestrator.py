# Hybrid orchestrator that provides interoperability between Terraform and gcloud, aws execution
import json
...
from .orchestrator import _generate_plan_dag, console, get_llm_interface
...
class HybridOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any], rag_engine: RAGEngine):
        ...
        self.default_executor = config.get(
            "default_executor", "gcloud"
        )  # 'gcloud', 'terraform', or 'hybrid'
        self.terraform_preferred_resources = config.get(
            "terraform_preferred_resources",
            [
        ...
        console.print("[green]Hybrid Orchestrator initialized[/green]")
        console.print(f"Default executor: {self.default_executor}")
        console.print(
            f"Terraform preferred for: {', '.join(self.terraform_preferred_resources)}"
        )

    async def process_query_hybrid(
        self, query: str, execution_mode: str = "auto", verbose: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if execution_mode == "dual":
                return await self._execute_dual_mode(
                    dag, step_details_map, state_logger, verbose
                )
            else:
                return await self._execute_single_mode(
                    dag, step_details_map, execution_strategy, state_logger, verbose
                )
    ...
    def _determine_execution_strategy(
        self, step_details_map: Dict[str, Any], execution_mode: str
    ) -> Dict[str, str]:
        """
    ...
    def _auto_select_executor(self, step_details: Dict[str, Any]) -> str:
        ...
        for tf_resource in self.terraform_preferred_resources:
            if (
                tf_resource.replace("_", " ") in description
                or tf_resource.replace("_", "-") in description
            ):
                return "terraform"
    ...
    def _display_execution_strategy(self, strategy: Dict[str, str]) -> None:
        ...
        strategy_table = Table(
            title="Execution Strategy", show_header=True, header_style="bold blue"
        )
        ...
        for step_id, executor in strategy.items():
            reason = (
                "User specified"
                if executor != self.default_executor
                else "Auto-selected"
            )
    ...
    async def _execute_single_mode(
        ...
        for step_id in execution_order:
            ...
            console.print(
                f"Description: {step_details.get('description', 'No description')}"
            )

            try:
                if executor_type == "terraform":
                    success, result = await self._execute_terraform_step(
                        step_id, step_details, verbose
                    )
                else:
                    success, result = await self._execute_gcloud_step(
                        step_id, step_details, verbose
                    )
    ...
    async def _execute_dual_mode(
        ...
        for step_id in execution_order:
            ...
            terraform_result = await self._execute_terraform_step(
                step_id, step_details, verbose
            )
            gcloud_result = await self._execute_gcloud_step(
                step_id, step_details, verbose
            )

            comparison = self._compare_execution_results(
                terraform_result, gcloud_result
            )
            ...
            results[step_id] = {
                ...
                "gcloud": {"success": gcloud_result[0], "result": gcloud_result[1]},
                "comparison": comparison,
            }

            self._display_dual_execution_result(step_id, results[step_id])

        return {"status": "completed", "results": results, "execution_mode": "dual"}

    async def _execute_terraform_step(
        self, step_id: str, step_details: Dict[str, Any], verbose: bool
    ) -> Tuple[bool, Any]:
    ...
    def _compare_execution_results(
        self, terraform_result: Tuple[bool, Any], gcloud_result: Tuple[bool, Any]
    ) -> Dict[str, Any]:
        ...
        if comparison["both_succeeded"]:
            if comparison["results_differ"]:
                comparison["recommendation"] = (
                    "Both succeeded but results differ - manual review needed"
                )
            else:
                comparison["recommendation"] = (
                    "Both succeeded with same result - either can be used"
                )
    ...
    def _display_dual_execution_result(
        ...
        try:
            ...
            if "nodes" in log_data:
                for node_id, node_data in log_data["nodes"].items():
                    if (
                        "output" in node_data
                        and "executed_command_str" in node_data["output"]
                    ):
                        command = node_data["output"]["executed_command_str"]

                        tf_config = (
                            self.terraform_executor._convert_gcloud_to_terraform(
                                command
                            )
                        )
                        ...
                        if (
                            tf_config
                            and tf_config.get("type") != "unsupported_conversion"
                        ):
                            terraform_configs[node_id] = tf_config
                            console.print(
                                f"[green]✓ Converted {node_id}: {command}[/green]"
                            )
                        else:
                            console.print(
                                f"[yellow]⚠ Could not convert {node_id}: {command}[/yellow]"
                            )
            ...
            if terraform_configs:
                ...
                console.print(
                    f"[green]Generated Terraform configuration: {output_file}[/green]"
                )
