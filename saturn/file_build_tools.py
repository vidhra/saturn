...
class FileBuildToolCaller:
    ...
    # Class-level cache for tools schema (shared across instances)
    ...
    def __init__(self, working_directory: str = "."):
        ...
        if self._is_tools_cache_valid():
            self.tools = self._tools_schema_cache.copy()
        else:
            self.tools = self._register_tools()
            # Update class cache
            FileBuildToolCaller._tools_schema_cache = self.tools.copy()
            FileBuildToolCaller._tools_cache_timestamp = time.time()

    @classmethod
    def _is_tools_cache_valid(cls) -> bool:
        """Check if the class-level tools cache is still valid."""
        return (
            cls._tools_schema_cache is not None
            and time.time() - cls._tools_cache_timestamp < cls._tools_cache_ttl
        )

    @classmethod
    def clear_tools_cache(cls):
        """Clear the class-level tools cache."""
        cls._tools_schema_cache = None
        cls._tools_cache_timestamp = 0
    ...
    async def read_file(self, file_path: str) -> Dict[str, Any]:
        ...
        success, result = await self.executor.execute(
            "read_file", {"file_path": file_path}, self.console, "read_file"
        )
        return {"success": success, "result": result, "tool": "read_file"}
    ...
    async def write_file(
        ...
        return {"success": success, "result": result, "tool": "write_file"}
    ...
    async def list_files(
        ...
        return {"success": success, "result": result, "tool": "list_files"}
    ...
    async def copy_file(self, source: str, destination: str) -> Dict[str, Any]:
        ...
        return {"success": success, "result": result, "tool": "copy_file"}

    async def template_file(
        self, template_path: str, output_path: str, variables: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        return {"success": success, "result": result, "tool": "template_file"}

    async def detect_project_type(self) -> Dict[str, Any]:
        """Auto-detect project type."""
        success, result = await self.executor.execute(
            "detect_project", {}, self.console, "detect_project"
        )
        return {"success": success, "result": result, "tool": "detect_project_type"}

    async def build_project(
        self, project_type: str = "auto", build_command: str = None
    ) -> Dict[str, Any]:
        ...
        success, result = await self.executor.execute(
            "build_project", params, self.console, "build_project"
        )
        return {"success": success, "result": result, "tool": "build_project"}

    async def test_project(
        self, project_type: str = "auto", test_command: str = None
    ) -> Dict[str, Any]:
        ...
        success, result = await self.executor.execute(
            "test_project", params, self.console, "test_project"
        )
        return {"success": success, "result": result, "tool": "test_project"}

    async def lint_project(
        self, project_type: str = "auto", lint_command: str = None
    ) -> Dict[str, Any]:
        ...
        success, result = await self.executor.execute(
            "lint_project", params, self.console, "lint_project"
        )
        return {"success": success, "result": result, "tool": "lint_project"}
    ...
    async def generate_dockerfile(
        ...
        success, result = await self.executor.execute(
            "generate_dockerfile", params, self.console, "generate_dockerfile"
        )
        return {"success": success, "result": result, "tool": "generate_dockerfile"}
    ...
    async def build_docker_image(
        ...
        return {"success": success, "result": result, "tool": "build_docker_image"}
    ...
    async def run_docker_container(
        ...
        success, result = await self.executor.execute(
            "run_docker", params, self.console, "run_docker"
        )
        return {"success": success, "result": result, "tool": "run_docker_container"}
    ...
    async def docker_compose_up(
        ...
        success, result = await self.executor.execute(
            "docker_compose",
            {"compose_file": compose_file, "detached": detached, "build": build},
            self.console,
            "docker_compose",
        )
        return {"success": success, "result": result, "tool": "docker_compose_up"}

    async def execute_command(
        self, command: str, working_directory: str = None
    ) -> Dict[str, Any]:
        """Execute a shell command."""
        success, result = await self.executor.execute(
            "execute_command",
            {"command": command, "working_directory": working_directory or "."},
            self.console,
            "execute_command",
        )
        return {"success": success, "result": result, "tool": "execute_command"}
    ...
    async def create_directory(
        ...
        return {"success": success, "result": result, "tool": "create_directory"}

    async def edit_file(
        self, file_path: str, instructions: str, code_edit: str
    ) -> Dict[str, Any]:
        success, result = await self.executor.execute(
            "edit_file",
            {
        ...
        return {"success": success, "result": result, "tool": "edit_file"}
...
class FileBuildToolHandler:
    ...
    def _parse_json_arguments_safely(self, arguments_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.console.print("[green]✓ Fixed JSON parsing with escaping[/green]")
                return result
    ...
    def _fix_json_escaping(self, json_str: str) -> str:
        """
        Fix JSON escaping issues, particularly with unescaped quotes in string values.
        This follows the approach used by OpenAI's function calling examples.
        """
        import re
